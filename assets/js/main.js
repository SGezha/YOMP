...
function start() {
	var AudioPlayer = (function () {
		...
		var player = document.getElementById('ap'),
			playBtn, prevBtn, video = false,
			nextBtn, plBtn, repeatBtn, volumeBtn, subtitlesBtn, progressBar, preloadBar, curTime, durTime, trackTitle, index = 0,
			wave = false,
			playList, volumeBar, videoBtn, volumeLength, repeating = false,
			random = false,
			seeking = false,
			rightClick = false,
			apActive = false,
			...
			settings = { volume: db().query("SELECT * from status")[0].volume ? db().query("SELECT * from status")[0].volume : 0.1, autoPlay: false, notification: true, playList: [] };
		...
		function listHandler(evt) {
			...
			if (aw == 'pl-title') {
				...
				const img = decodeURI(playList[index].icon);

				ColorThief.getPalette(img, 2)
					.then(color => {
						document.documentElement.style.setProperty('--purple', `rgb(${color[0][0]}, ${color[0][1]}, ${color[0][2]})`);
						document.documentElement.style.setProperty('--red', `rgb(${color[1][0]}, ${color[1][1]}, ${color[1][2]})`);
					})
