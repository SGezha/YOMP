...
function getText() {
  ...
  timer = setInterval(() => {
    if (text.find(a => Number(a._attributes.start).toFixed(0) == audio.currentTime.toFixed(0)) != undefined) {
      stroka = text.find(a => Number(a._attributes.start).toFixed(0) == audio.currentTime.toFixed(0))._text ? text.find(a => Number(a._attributes.start).toFixed(0) == audio.currentTime.toFixed(0))._text : "" ;
      if (!remote.getCurrentWindow().isFocused()) {
        require('electron').ipcRenderer.send("notification", {body: text.find(a => Number(a._attributes.start).toFixed(0) == audio.currentTime.toFixed(0))._text, img: ""})
      };
    }
