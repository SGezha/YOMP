	...
	var AudioPlayer = (function () {
		...
		function renderPL() {
			...
			playList.forEach(function (item, i) {
				...
				if (i < 80) {
					html.push(`<li class="music-el" real-id="${item.id}" data-track="${i}"><div class="pog" real-id="${item.id}" data-track="${i}"><div class="left"><div class="pl-number"><div class="pl-count">${type}</div><div class="pl-playing"><img src="assets/icons/music.gif"></div></div></div><div class="center" real-id="${item.id}" data-track="${i}"><div class="pl-title">${item.title}</div></div><div class="right" real-id="${item.id}" data-track="${i}"><div class="music-id">${i + 1}/${playList.length}</div>${fav}<i class="fas fa-trash pl-remove pl-del"></i></div><div class="music-background" style="background: url('${item.icon}') center center / cover;"></div></div></li>`);
				} else {
